...
class BaseHandler(Handler):
    fn __init__(self, name): self._name = name   # protected by convention
    fn _log(self, req): print(f"[{self._name}] {req.method} {req.path}")
...
class Router:
    ...
    fn register(self, method, pattern, handler):
        self.__routes[(method, pattern)] = handler  # method + path key

    fn dispatch(self, method, url):
        parts = urlsplit(url)
